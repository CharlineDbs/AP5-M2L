...
class Formulaire{
	...
	public function creerTextArea($unNom, $unId, $uneValue , $rows, $cols, $required , $placeholder , $readOnly){
		$composant = "<textarea type = 'textarea' name = '" . $unNom . "' id = '" . $unId . "' ";
		if (!empty($uneValue)){
			$composant .= "value = '" . $uneValue . "' ";
		}
		if (!empty($rows)){
	...
	public function creerImage($unNomLigue, $style){
		$composant = "<img src= 'images/" . $unNomLigue.".png' class = '" . $style ."' />";
		return $composant;
	}
